...
if 'Reading' not in svars:
    print('Student did not copy Reading file')
else:
    if not callable(scode.Reading):
        print('Student overwrote Reading file')
...
if 'readings_after' not in svars:
    ...
    if not good_annotations:
        try:
            res = scode.readings_after(cutoffdate, rs)
        except TypeError:
            try:
                res = scode.readings_after(rs, cutoffdate)
            except:
    ...
    if res == tacode.readings_after(cd, rs):
        q3 = True
    else:
        print('Potentially got dates before, not dates after.')
        q3 = False
